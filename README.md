<!-- Header -->
![Header](https://capsule-render.vercel.app/api?type=waving&color=00bfbf&height=120&section=header)

[![Typing SVG](https://readme-typing-svg.herokuapp.com/?color=green&size=35&center=true&vCenter=true&width=1000&lines=Olá,+meu+nome+é+Alexandre+Silva+Felismino;Tenho+22+anos;Sou+do+Brasil,+RJ;Estudo+Análise+e+Desenvolvimento+de+Sistemas;+Seja+bem+vindo!)](https://git.io/typing-svg)

<div align="center">
  <!-- GitHub Readme Stats (card) -->
  <img width="49%" height="195px" src="https://github-readme-stats.vercel.app/api?username=xande-rj&show_icons=true&count_private=true&hide_border=true&theme=transparent&title_color=00bfbf&icon_color=00bfbf&text_color=c9d1d9&bg_color=0d1117" alt="Alexandre's GitHub stats" />
  <!-- Top Languages (compact) -->
  <img width="41%" height="195px" src="https://github-readme-stats.vercel.app/api/top-langs/?username=xande-rj&layout=compact&hide_border=true&langs_count=8&theme=transparent" alt="Top languages" />
</div>

---

# 🚀 Alexandre Silva Felismino

Desenvolvedor focado em **Back-End (Java / Spring Boot)** com experiência em construção de APIs REST, autenticação (JWT), bancos relacionais (PostgreSQL), containerização (Docker) e testes automatizados. Também atuo com Node.js (TypeScript) e tenho conhecimentos em frontend com React.

---

## 🔎 Pontos de interesse (o que eu trago)
- **APIs seguras e escaláveis:** experiência com Spring Boot, Spring Security e JWT.  
- **Persistência robusta:** modelagem e otimização com PostgreSQL / MySQL.  
- **Qualidade de código:** testes unitários e de integração (JUnit, Jest).  
- **Containerização & Deploy:** Docker, Docker Compose e conceitos de CI/CD.  
- **Integração Front-End:** consumo de APIs com React e boas práticas REST.  
- **Boa comunicação e trabalho em equipe**, sempre aberto a aprender e aplicar novas tecnologias.

---

## 🛠️ Tecnologias e Ferramentas (ícones de foco)
![Java](https://img.shields.io/badge/Java-0D1117?style=for-the-badge&logo=java&labelColor=0D1117)&nbsp;
![Spring Boot](https://img.shields.io/badge/Spring_Boot-0D1117?style=for-the-badge&logo=spring&labelColor=0D1117)&nbsp;
![Node.js](https://img.shields.io/badge/Node.js-0D1117?style=for-the-badge&logo=node.js&labelColor=0D1117)&nbsp;
![TypeScript](https://img.shields.io/badge/TypeScript-0D1117?style=for-the-badge&logo=typescript&labelColor=0D1117)&nbsp;
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-0D1117?style=for-the-badge&logo=postgresql&labelColor=0D1117)&nbsp;
![Docker](https://img.shields.io/badge/Docker-0D1117?style=for-the-badge&logo=docker&labelColor=0D1117)&nbsp;
![React](https://img.shields.io/badge/React-0D1117?style=for-the-badge&logo=react&labelColor=0D1117)&nbsp;

---

## 🧠 Outros conhecimentos
![PHP](https://img.shields.io/badge/PHP-0D1117?style=for-the-badge&logo=php&labelColor=0D1117)&nbsp;
![MySQL](https://img.shields.io/badge/MySQL-0D1117?style=for-the-badge&logo=mysql&labelColor=0D1117)&nbsp;
![Figma](https://img.shields.io/badge/Figma-0D1117?style=for-the-badge&logo=figma&labelColor=0D1117)&nbsp;
![GitHub](https://img.shields.io/badge/GitHub-0D1117?style=for-the-badge&logo=github&labelColor=0D1117)&nbsp;
![VS Code](https://img.shields.io/badge/VS_Code-0D1117?style=for-the-badge&logo=visual-studio-code&labelColor=0D1117)&nbsp;

---

## 📚 Atualmente estudando
![NestJS](https://img.shields.io/badge/NestJS-0D1117?style=for-the-badge&logo=nestjs&labelColor=0D1117)&nbsp;
![Laravel](https://img.shields.io/badge/Laravel-0D1117?style=for-the-badge&logo=laravel&labelColor=0D1117)&nbsp;

---

## ✨ Projetos em destaque
### 1) Serviço de Autenticação — Spring Boot
- **Stack:** Java, Spring Boot, Spring Security, JWT, PostgreSQL  
- **Recursos:** login, refresh token, roles, persistência e testes.  
- **Docs:** OpenAPI / README com exemplos de requests.

### 2) Encurtador de URL — API REST (Java)
- **Stack:** Spring Boot, JPA/Hibernate, PostgreSQL  
- **Recursos:** geração de short URL, redirecionamento, estatísticas simples.

### 3) API em Node.js (TypeScript)
- **Stack:** Node.js, TypeScript, Express/NestJS (estudo)  
- **Recursos:** validação, autenticação JWT, testes e Docker.

> Veja os READMEs específicos em cada repositório para instruções completas, Postman/Insomnia e exemplos de uso.

---

## 📫 Contato
- ✉️ alexandrefelismino@gmail.com  
- 📱 +55 (21) 99126-7959  
- 🔗 GitHub: https://github.com/xande-rj  
- 🔗 LinkedIn: https://www.linkedin.com/in/alexandresilvafelismino

---

![Footer](https://capsule-render.vercel.app/api?type=waving&color=00bfbf&height=120&section=footer)
